  ...
  <div className="bg-white border-b border-slate-200 py-2 px-4 sticky top-0 z-50 flex items-center justify-between shadow-sm">
    <div className="flex items-center gap-2">
      ...
      <div className="flex items-center gap-1 font-extrabold text-slate-900 text-[10px] tracking-tight uppercase">
        ...
        <span className="text-slate-300">/</span>
        <span className="text-black">MCPEDL Downloader</span>
      </div>
    </div>
  ...
  <div className="bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm">
    <table className="w-full text-left border-collapse">
      ...
      <tbody className="text-[9px]">
        <ParamRow param="url" type="String" desc="URL atau Slug postingan dari situs MCPEDL." required />
        <ParamRow param="downloadFile" type="Boolean" desc="Download file modifikasinya? (default: true)." />
        <ParamRow param="mainDownloadOnly" type="Boolean" desc="Hanya download file utamanya saja? (default: true)." />
      </tbody>
    </table>
  </div>
);
...
export default function MCPEDLDocs() {
  const [urlParam, setUrlParam] = useState("https://mcpedl.com/lunac-shaders/");
  const [downloadFileParam, setDownloadFileParam] = useState("true");
  const [mainDownloadOnlyParam, setMainDownloadOnlyParam] = useState("true");
  ...
  const handleExecute = async () => {
    ...
    try {
      const url = new URL("/api/downloader/mcpedl", window.location.origin);
      url.searchParams.set("url", urlParam);
      url.searchParams.set("downloadFile", downloadFileParam);
      url.searchParams.set("mainDownloadOnly", mainDownloadOnlyParam);
      ...
      <main className="max-w-[700px] mx-auto px-4 py-8 space-y-10 pb-20">
        ...
        <section className="space-y-4">
          <div className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-gray-100 border border-gray-200/50 rounded-[4px] text-[7px] font-black text-black uppercase tracking-[0.2em] shadow-sm">
            <Box className="w-2.5 h-2.5" /> MCPEDL NODE
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-[20px] font-black text-slate-900 tracking-tight uppercase italic leading-none flex items-center gap-2">
              MCPEDL DL <span className="text-slate-400">Node</span>
            </h1>
          </div>

          <p className="text-[12px] text-slate-600 leading-relaxed max-w-xl pb-2">
            Tool scraper untuk mengunduh mod/addons/maps dari MCPEDL beserta detail infonya secara otomatis, mendukung smart select link utama.
          </p>

          <EndpointBox method="GET" url="https://stenly.org/api/downloader/mcpedl" />
        </section>
                    ...
                    >
                      <div className="space-y-1.5">
                        ...
                        <div className="text-sky-400 text-[10px] font-bold leading-relaxed border-l-2 border-sky-900/50 pl-3 py-0.5 break-all">
                          https://stenly.org/api/downloader/mcpedl?url={encodeURIComponent(urlParam)}&downloadFile={downloadFileParam}&mainDownloadOnly={mainDownloadOnlyParam}
                        </div>
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <pre className="text-[10px] font-bold leading-loose p-5 bg-[#0a0a0c] rounded-xl border border-slate-900 shadow-inner overflow-x-auto whitespace-pre-wrap">
                          ...
                          &nbsp;&nbsp;<span className="text-indigo-400">&quot;downloadFile&quot;</span><span className="text-slate-600">:</span> <span className="text-emerald-500">&quot;{downloadFileParam}&quot;</span>,<br />
                          &nbsp;&nbsp;<span className="text-indigo-400">&quot;mainDownloadOnly&quot;</span><span className="text-slate-600">:</span> <span className="text-emerald-500">&quot;{mainDownloadOnlyParam}&quot;</span><br />
             ...
             <div className="p-4 bg-[#0a0a0c] border-t border-slate-900 shrink-0 space-y-4">
                ...
                <div className="flex flex-col gap-3 px-2">
                   <div className="flex items-center gap-2 flex-1">
                     <span className="text-slate-600 text-[10px] font-bold uppercase tracking-widest w-16">URL/SLUG</span>
                     <input 
                       type="text"
                       value={urlParam}
                       ...
                       placeholder="https://mcpedl.com/lunac-shaders/"
                     />
                   </div>
                   
                   <div className="flex gap-4">
                     <div className="flex items-center gap-2 flex-1">
                       <span className="text-slate-600 text-[10px] font-bold uppercase tracking-widest w-24">DOWNLOAD FILE</span>
                       <select 
                         value={downloadFileParam}
                         onChange={(e) => setDownloadFileParam(e.target.value)}
                     ...
                     <div className="flex items-center gap-2 flex-1">
                       <span className="text-slate-600 text-[10px] font-bold uppercase tracking-widest w-24">MAIN ONLY</span>
                       <select 
                         value={mainDownloadOnlyParam}
                         onChange={(e) => setMainDownloadOnlyParam(e.target.value)}
                         className="bg-[#111] border border-slate-800 text-[10px] font-bold text-slate-300 outline-none w-full p-1 rounded transition-all focus:border-sky-600"
                       >
                         <option value="true">True</option>
                         <option value="false">False</option>
