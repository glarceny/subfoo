  ...
  <div className="bg-white border-b border-slate-200 py-2 px-4 sticky top-0 z-50 flex items-center justify-between shadow-sm">
    <div className="flex items-center gap-2">
      ...
      <div className="flex items-center gap-1 font-extrabold text-slate-900 text-[10px] tracking-tight uppercase">
        ...
        <span className="text-slate-300">/</span>
        <span className="text-black">ImgToPrompt</span>
      </div>
    </div>
  ...
  <div className="bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm">
    <table className="w-full text-left border-collapse">
      ...
      <tbody className="text-[9px]">
        <ParamRow param="url" type="String" desc="URL gambar yang ingin digenerate prompt-nya." required />
      </tbody>
    </table>
  </div>
);
...
export default function ImgToPromptDocs() {
  const [urlParam, setUrlParam] = useState("https://picsum.photos/seed/picsum/200/300");
  ...
  const handleExecute = async () => {
    ...
    try {
      const url = new URL("/api/ai/imgtoprompt", window.location.origin);
      ...
      <main className="max-w-[700px] mx-auto px-4 py-8 space-y-10 pb-20">
        ...
        <section className="space-y-4">
          <div className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-gray-100 border border-gray-200/50 rounded-[4px] text-[7px] font-black text-black uppercase tracking-[0.2em] shadow-sm">
            <ImageIcon className="w-2.5 h-2.5" /> AI NODE
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-[20px] font-black text-slate-900 tracking-tight uppercase italic leading-none flex items-center gap-2">
              ImgToPrompt <span className="text-slate-400">Node</span>
            </h1>
          </div>

          <p className="text-[12px] text-slate-600 leading-relaxed max-w-xl pb-2">
            Generate prompt dari gambar menggunakan model AI Nano Banana Pro.
          </p>

          <EndpointBox method="GET" url="https://stenly.org/api/ai/imgtoprompt" />
        </section>
            ...
            <div className="space-y-4 flex-1 min-w-0">
              ...
              <div className="space-y-4 w-full pt-1">
                <MicroCodeBlock 
                  ...
                  rawCode={`curl "https://stenly.org/api/ai/imgtoprompt?url=https://picsum.photos/seed/picsum/200/300"`}
                  copyId="curl"
                  copied={copied}
                  onCopy={copyCode}
                >
                  <span className="text-emerald-400">curl</span> <span className="text-emerald-300">&quot;https://stenly.org/api/ai/imgtoprompt?url=https://picsum.photos/seed/picsum/200/300&quot;</span>
                </MicroCodeBlock>

                <MicroCodeBlock 
                  label="Node.js (Fetch)"
                  rawCode={`const res = await fetch("https://stenly.org/api/ai/imgtoprompt?url=" + encodeURIComponent("https://picsum.photos/seed/picsum/200/300"));\nconst data = await res.json();\nconsole.log(data);`}
                  copyId="js"
                  copied={copied}
                  onCopy={copyCode}
                >
                  <span className="text-indigo-400">const</span> res = <span className="text-indigo-400">await</span> <span className="text-sky-400">fetch</span>(<span className="text-emerald-300">&quot;https://stenly.org/api/ai/imgtoprompt?url=&quot;</span> + <span className="text-sky-400">encodeURIComponent</span>(<span className="text-emerald-300">&quot;https://picsum.photos/seed/picsum/200/300&quot;</span>));<br/>
                    ...
                    >
                      <div className="space-y-1.5">
                        ...
                        <div className="text-sky-400 text-[10px] font-bold leading-relaxed border-l-2 border-sky-900/50 pl-3 py-0.5 break-all">
                          https://stenly.org/api/ai/imgtoprompt?url={urlParam}
                        </div>
                      </div>
