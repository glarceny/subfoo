...
const Header = () => (
  <div className="bg-white border-b border-slate-200 py-2 px-4 sticky top-0 z-50 flex items-center justify-between shadow-sm">
    <div className="flex items-center gap-2">
      <div className="w-4 h-4 bg-emerald-600 rounded flex items-center justify-center text-white font-black text-[8px] shadow-inner">W</div>
      <div className="flex items-center gap-1 font-extrabold text-slate-900 text-[10px] tracking-tight uppercase">
        ...
        <span className="text-slate-300">/</span>
        <span className="text-emerald-600">Wise-Currency</span>
      </div>
    </div>
  ...
  <div className="bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm">
    <table className="w-full text-left border-collapse">
      ...
      <tbody className="text-[9px]">
        <ParamRow param="mode" type="String" desc="'convert' untuk konversi nilai, 'currencies' untuk daftar mata uang." />
        <ParamRow param="from" type="String" desc="Kode mata uang asal (misal: USD, EUR, IDR)." />
        <ParamRow param="to" type="String" desc="Kode mata uang tujuan (misal: IDR, USD)." />
        <ParamRow param="amount" type="Number" desc="Jumlah uang yang ingin dikonversi (default 1)." />
        <ParamRow param="days" type="Number" desc="Jumlah hari history grafik untuk dirender (default 30)." />
      </tbody>
    </table>
  </div>
);
...
export default function WiseDocsUltraDense() {
  ...
  const handleExecute = async () => {
    ...
    try {
      const url = new URL("/api/wise", window.location.origin);
    ...
    <div className="min-h-screen bg-[#fafafa] font-sans text-slate-900 selection:bg-sky-200">
      ...
      <main className="max-w-[700px] mx-auto px-4 py-8 space-y-10 pb-20">
        ...
        <section className="space-y-4">
          <div className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-emerald-50 border border-emerald-100/50 rounded-[4px] text-[7px] font-black text-emerald-600 uppercase tracking-[0.2em] shadow-sm">
            <CircleDollarSign className="w-2.5 h-2.5" /> CURRENCY NODE
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-[20px] font-black text-slate-900 tracking-tight uppercase italic leading-none flex items-center gap-2">
              Wise Currency <span className="text-slate-400">Node</span>
            </h1>
          </div>

          <p className="text-[12px] text-slate-600 leading-relaxed max-w-xl pb-2">
            API Konversi Mata Uang real-time dengan integrasi history grafik. Menarik kurs terkini dan riwayat dari Wise, beserta <span className="font-semibold text-emerald-600">Chart Generator</span> untuk mendapatkan gambar grafik pertukaran.
          </p>

          <EndpointBox method="GET" url="https://stenly.org/api/wise" />
        </section>
        ...
        <section className="space-y-5">
          ...
          <div className="flex gap-3 items-start w-full">
            ...
            <div className="space-y-4 flex-1 min-w-0">
              ...
              <p className="text-[10px] text-slate-600 leading-relaxed -mt-2">Lakukan HTTP GET request ke endpoint dengan query params yang dibutuhkan.</p>
              
              <div className="space-y-4 w-full pt-1">
                <MicroCodeBlock 
                  label="Terminal / Bash (cURL - GET)"
                  rawCode={`curl "https://stenly.org/api/wise?mode=convert&from=USD&to=IDR&amount=10&days=30"`}
                  copyId="curl"
                  copied={copied}
                  onCopy={copyCode}
                >
                  <span className="text-emerald-400">curl</span> <span className="text-emerald-300">&quot;https://stenly.org/api/wise?mode=convert&amp;from=USD&amp;to=IDR&amp;amount=10&amp;days=30&quot;</span>
                </MicroCodeBlock>

                <MicroCodeBlock 
                  label="Node.js (Fetch)"
                  rawCode={`const res = await fetch("https://stenly.org/api/wise?mode=convert&from=USD&to=IDR&amount=10");\nconst data = await res.json();\nconsole.log(data);`}
                  copyId="js"
                  copied={copied}
                  onCopy={copyCode}
                >
                  <span className="text-indigo-400">const</span> res = <span className="text-indigo-400">await</span> <span className="text-sky-400">fetch</span>(<span className="text-emerald-300">&quot;https://stenly.org/api/wise?mode=convert&amp;from=USD&amp;to=IDR&amp;amount=10&quot;</span>);<br/>
        ...
        <section className="space-y-4 pt-4">
          <div className="flex items-center gap-2 mb-3 border-b border-emerald-100 pb-2">
            ...
            <h2 className="text-[10px] font-black text-emerald-900 uppercase tracking-widest italic">Hasil Yang Didapatkan (Response)</h2>
          </div>

          <div className="bg-white border border-emerald-100 rounded-xl p-5 shadow-sm space-y-6">
            ...
            <div className="space-y-2">
               ...
               <div className="bg-[#050505] rounded-xl p-4 border border-slate-800 shadow-inner overflow-x-auto">
                 <pre className="font-mono font-bold text-[9px] leading-loose">
                   ...
                   <div className="text-emerald-500 pl-4">&quot;Code&quot;: <span className="text-amber-500">200</span>,</div>
                   <div className="text-emerald-500 pl-4">&quot;Amount&quot;: <span className="text-amber-500">10</span>,</div>
                   <div className="text-emerald-500 pl-4">&quot;From&quot;: <span className="text-emerald-300">&quot;USD&quot;</span>,</div>
                   <div className="text-emerald-500 pl-4">&quot;To&quot;: <span className="text-emerald-300">&quot;IDR&quot;</span>,</div>
                   <div className="text-emerald-500 pl-4">&quot;Rate&quot;: <span className="text-amber-500">16124.5</span>,</div>
                   <div className="text-emerald-500 pl-4">&quot;Result&quot;: <span className="text-amber-500">161245</span>,</div>
                   <div className="text-emerald-500 pl-4">&quot;Chart_url&quot;: <span className="text-emerald-300">&quot;https://quickchart.io/chart/maker/123...&quot;</span>,</div>
                   <div className="text-emerald-500 pl-4">&quot;Time_ms&quot;: <span className="text-amber-500">1204</span></div>
                   <div className="text-emerald-500">{"}"}</div>
                 </pre>
               </div>
            </div>
                    ...
                    >
                      <div className="space-y-1.5">
                        ...
                        <div className="text-sky-400 text-[10px] font-bold leading-relaxed border-l-2 border-sky-900/50 pl-3 py-0.5 break-all">
                          https://stenly.org/api/wise?mode={mode}{mode === 'convert' ? `&from=${from}&to=${to}&amount=${amount}` : ''}
                        </div>
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <pre className="text-[10px] font-bold leading-loose p-5 bg-[#0a0a0c] rounded-xl border border-slate-900 shadow-inner">
                          ...
                          &nbsp;&nbsp;<span className="text-indigo-400">&quot;mode&quot;</span><span className="text-slate-600">:</span> <span className="text-emerald-500">&quot;{mode}&quot;</span>{mode === 'convert' && <span className="text-slate-500">,</span>}<br />
                          {mode === 'convert' && (
                            <>
                              &nbsp;&nbsp;<span className="text-indigo-400">&quot;from&quot;</span><span className="text-slate-600">:</span> <span className="text-emerald-500">&quot;{from}&quot;</span><span className="text-slate-500">,</span><br />
                              &nbsp;&nbsp;<span className="text-indigo-400">&quot;to&quot;</span><span className="text-slate-600">:</span> <span className="text-emerald-500">&quot;{to}&quot;</span><span className="text-slate-500">,</span><br />
                              &nbsp;&nbsp;<span className="text-indigo-400">&quot;amount&quot;</span><span className="text-slate-600">:</span> <span className="text-amber-500">{amount}</span><br />
                            </>
                          )}
                    ...
                    >
                      ...
                      {isLoading ? (
                         ...
                         <div className="space-y-3">
                           ...
                           {response.Chart_url && (
                             <div className="p-3 border border-slate-800 rounded-xl bg-[#0a0a0c]">
                               <img src={response.Chart_url} alt="Chart" className="w-full rounded-lg" />
                             </div>
                           )}
                         </div>
                      ) : (
             ...
             <div className="p-4 bg-[#0a0a0c] border-t border-slate-900 shrink-0 space-y-4">
                ...
                <div className="flex items-center justify-center pt-1">
                   <button 
                     ...
                     disabled={isLoading || (mode === 'convert' && (!from || !to || !amount))}
                ...
                <div className="flex items-center justify-between gap-2 px-2">
                   <select 
                     ...
                     className="bg-transparent border-b border-slate-800 text-[10px] font-bold text-slate-300 outline-none transition-all focus:border-sky-600 pb-1 w-24 uppercase"
                   >
                     <option value="convert" className="bg-[#0a0a0c]">Convert</option>
                     <option value="currencies" className="bg-[#0a0a0c]">Currencies</option>
                   </select>
                   ...
                   {mode === "convert" && (
                     <div className="flex items-center gap-2 flex-1 justify-end">
                       <input 
                         ...
                         className="bg-transparent border-b border-slate-800 text-[10px] font-bold text-slate-300 outline-none w-12 text-center transition-all focus:border-sky-600 placeholder:text-slate-700 pb-1"
                         placeholder="FROM"
                       />
                       ...
                       <input 
                         ...
                         className="bg-transparent border-b border-slate-800 text-[10px] font-bold text-slate-300 outline-none w-16 text-right transition-all focus:border-sky-600 placeholder:text-slate-700 pb-1"
                         placeholder="Amount"
                       />
                     </div>
                   )}
