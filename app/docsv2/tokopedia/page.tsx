  ...
  <div className="bg-white border-b border-slate-200 py-2 px-4 sticky top-0 z-50 flex items-center justify-between shadow-sm">
    <div className="flex items-center gap-2">
      ...
      <div className="flex items-center gap-1 font-extrabold text-slate-900 text-[10px] tracking-tight uppercase">
        ...
        <span className="text-slate-300">/</span>
        <span className="text-emerald-600">Tokopedia</span>
      </div>
    </div>
  ...
  <div className="bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm">
    <table className="w-full text-left border-collapse">
      ...
      <tbody className="text-[9px]">
        <ParamRow param="query" type="String" desc="Kata kunci yang ingin dicari informasinya (misal: 'sejarah indonesia')." required />
        <ParamRow param="page" type="Number" desc="Halaman pencarian (default: 1)." />
        <ParamRow param="rows" type="Number" desc="Jumlah data yang diminta dari Tokopedia (default: 60)." />
        <ParamRow param="limit" type="Number" desc="Jumlah data yang ditampilkan pada output (default: 2)." />
      </tbody>
    </table>
  </div>
);
...
export default function TokopediaDocsUltraDense() {
  const [query, setQuery] = useState("vivo x300");
  ...
  const handleExecute = async () => {
    ...
    try {
      const url = new URL("/api/tokopedia", window.location.origin);
    ...
    <div className="min-h-screen bg-[#fafafa] font-sans text-slate-900 selection:bg-sky-200">
      ...
      <main className="max-w-[700px] mx-auto px-4 py-8 space-y-10 pb-20">
        ...
        <section className="space-y-4">
          <div className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-green-50 border border-green-100/50 rounded-[4px] text-[7px] font-black text-green-600 uppercase tracking-[0.2em] shadow-sm">
            <ShoppingBag className="w-2.5 h-2.5" /> E-COMMERCE NODE
          </div>
          
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-green-100 rounded-xl flex items-center justify-center shrink-0 border border-green-200">
            ...
            <h1 className="text-[20px] font-black text-slate-900 tracking-tight uppercase italic leading-none flex items-center gap-2">
              Tokopedia Search <span className="text-slate-400">Node</span>
            </h1>
          </div>

          <p className="text-[12px] text-slate-600 leading-relaxed max-w-xl pb-2">
            API pencarian produk Tokopedia secara real-time. Mengambil daftar produk lengkap beserta info diskon, harga, kategori, badge, dan info toko.
          </p>

          <EndpointBox method="GET" url="https://stenly.org/api/tokopedia" />
        </section>
        ...
        <section className="space-y-5">
          ...
          <div className="flex gap-3 items-start w-full">
            ...
            <div className="space-y-4 flex-1 min-w-0">
              ...
              <p className="text-[10px] text-slate-600 leading-relaxed -mt-2">Lakukan HTTP GET request ke endpoint dengan query param yang dibutuhkan.</p>
              
              <div className="space-y-4 w-full pt-1">
                <MicroCodeBlock 
                  label="Terminal / Bash (cURL - GET)"
                  rawCode={`curl "https://stenly.org/api/tokopedia?query=vivo%20x300&page=1&limit=2"`}
                  copyId="curl"
                  copied={copied}
                  onCopy={copyCode}
                >
                  <span className="text-emerald-400">curl</span> <span className="text-emerald-300">&quot;https://stenly.org/api/tokopedia?query=vivo%20x300&amp;page=1&amp;limit=2&quot;</span>
                </MicroCodeBlock>

                <MicroCodeBlock 
                  label="Node.js (Fetch)"
                  rawCode={`const res = await fetch("https://stenly.org/api/tokopedia?query=vivo%20x300&page=1&limit=2");\nconst data = await res.json();\nconsole.log(data);`}
                  copyId="js"
                  copied={copied}
                  onCopy={copyCode}
                >
                  <span className="text-indigo-400">const</span> res = <span className="text-indigo-400">await</span> <span className="text-sky-400">fetch</span>(<span className="text-emerald-300">&quot;https://stenly.org/api/tokopedia?query=vivo%20x300&amp;page=1&amp;limit=2&quot;</span>);<br/>
        ...
        <section className="space-y-4 pt-4">
          <div className="flex items-center gap-2 mb-3 border-b border-green-100 pb-2">
            ...
            <h2 className="text-[10px] font-black text-green-900 uppercase tracking-widest italic">Hasil Yang Didapatkan (Response)</h2>
          </div>

          <div className="bg-white border border-green-100 rounded-xl p-5 shadow-sm space-y-6">
            ...
            <div className="space-y-2">
               ...
               <div className="bg-[#050505] rounded-xl p-4 border border-slate-800 shadow-inner overflow-x-auto">
                 <pre className="font-mono font-bold text-[9px] leading-loose">
                   ...
                   <div className="text-green-500 pl-4">&quot;Status&quot;: <span className="text-sky-400">true</span>,</div>
                   <div className="text-green-500 pl-4">&quot;Code&quot;: <span className="text-amber-500">200</span>,</div>
                   <div className="text-green-500 pl-4">&quot;Result&quot;: [ ... ]</div>
                   <div className="text-green-500">{"}"}</div>
                 </pre>
               </div>
            </div>
                    ...
                    >
                      <div className="space-y-1.5">
                        ...
                        <div className="text-sky-400 text-[10px] font-bold leading-relaxed border-l-2 border-sky-900/50 pl-3 py-0.5 break-all">
                          https://stenly.org/api/tokopedia?query={encodeURIComponent(query)}&page={page}&rows={rows}&limit={limit}
                        </div>
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <pre className="text-[10px] font-bold leading-loose p-5 bg-[#0a0a0c] rounded-xl border border-slate-900 shadow-inner">
                          ...
                          &nbsp;&nbsp;<span className="text-indigo-400">&quot;query&quot;</span><span className="text-slate-600">:</span> <span className="text-emerald-500">&quot;{query}&quot;</span><span className="text-slate-500">,</span><br />
                          &nbsp;&nbsp;<span className="text-indigo-400">&quot;page&quot;</span><span className="text-slate-600">:</span> <span className="text-amber-500">{page}</span><span className="text-slate-500">,</span><br />
                          &nbsp;&nbsp;<span className="text-indigo-400">&quot;rows&quot;</span><span className="text-slate-600">:</span> <span className="text-amber-500">{rows}</span><span className="text-slate-500">,</span><br />
                          &nbsp;&nbsp;<span className="text-indigo-400">&quot;limit&quot;</span><span className="text-slate-600">:</span> <span className="text-amber-500">{limit}</span><br />
                    ...
                    >
                      ...
                      {isLoading ? (
                         ...
                         <div className="space-y-3">
                           <pre className="text-[10px] font-bold leading-relaxed text-green-500 bg-[#0a0a0c] p-5 rounded-xl border border-slate-900 shadow-inner overflow-x-auto whitespace-pre-wrap">
             ...
             <div className="p-4 bg-[#0a0a0c] border-t border-slate-900 shrink-0 space-y-4">
                ...
                <div className="flex items-center justify-between gap-2 px-2">
                   <div className="flex items-center gap-2 flex-1 justify-center">
                     <span className="text-slate-600 text-[10px] font-bold">Q:</span>
                     <input 
                       type="text"
                       value={query}
                       onChange={(e) => setQuery(e.target.value)}
                       className="bg-transparent border-b border-slate-800 text-[10px] font-bold text-slate-300 outline-none w-full max-w-[200px] text-center transition-all focus:border-sky-600 placeholder:text-slate-700 pb-1"
                     ...
                     <input 
                       ...
                       className="bg-transparent border-b border-slate-800 text-[10px] font-bold text-slate-300 outline-none w-16 text-center transition-all focus:border-sky-600 placeholder:text-slate-700 pb-1 ml-2"
                       placeholder="Limit"
                     />
                   </div>
                </div>
