...
const Header = () => (
  <div className="bg-white border-b border-slate-200 py-2 px-4 sticky top-0 z-50 flex items-center justify-between shadow-sm">
    <div className="flex items-center gap-2">
      <div className="w-4 h-4 bg-black rounded flex items-center justify-center text-white font-black text-[8px] shadow-inner">H</div>
      <div className="flex items-center gap-1 font-extrabold text-slate-900 text-[10px] tracking-tight uppercase">
        ...
        <span className="text-slate-300">/</span>
        <span className="text-black">Honcho AI</span>
      </div>
    </div>
  ...
  <div className="bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm">
    <table className="w-full text-left border-collapse">
      ...
      <tbody className="text-[9px]">
        <ParamRow param="text" type="String" desc="Pesan pertanyaan ke Honcho AI (claude-sonnet-4-5-20250929)." required />
      </tbody>
    </table>
  </div>
);
...
export default function HonchoDocs() {
  const [textParam, setTextParam] = useState("halo, siapa kamu?");
  ...
  const handleExecute = async () => {
    ...
    try {
      const url = new URL("/api/ai/honcho", window.location.origin);
        ...
        <section className="space-y-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-[20px] font-black text-slate-900 tracking-tight uppercase italic leading-none flex items-center gap-2">
              Honcho AI <span className="text-slate-400">Node</span>
            </h1>
          </div>

          <p className="text-[12px] text-slate-600 leading-relaxed max-w-xl pb-2">
            Endpoint AI dari Honcho menggunakan model claude-sonnet-4-5-20250929 yang powerful untuk menjawab pertanyaan Anda.
          </p>

          <EndpointBox method="GET" url="https://stenly.org/api/ai/honcho" />
        </section>
        ...
        <section className="space-y-5">
          ...
          <div className="flex gap-3 items-start w-full">
            ...
            <div className="space-y-4 flex-1 min-w-0">
              ...
              <p className="text-[10px] text-slate-600 leading-relaxed -mt-2">Lakukan HTTP GET request ke endpoint dengan mengirim parameter `text`.</p>
              
              <div className="space-y-4 w-full pt-1">
                <MicroCodeBlock 
                  label="Terminal / Bash (cURL - GET)"
                  rawCode={`curl "https://stenly.org/api/ai/honcho?text=halo"`}
                  copyId="curl"
                  copied={copied}
                  onCopy={copyCode}
                >
                  <span className="text-emerald-400">curl</span> <span className="text-emerald-300">&quot;https://stenly.org/api/ai/honcho?text=halo&quot;</span>
                </MicroCodeBlock>

                <MicroCodeBlock 
                  label="Node.js (Fetch)"
                  rawCode={`const res = await fetch("https://stenly.org/api/ai/honcho?text=" + encodeURIComponent("halo"));\nconst data = await res.json();\nconsole.log(data);`}
                  copyId="js"
                  copied={copied}
                  onCopy={copyCode}
                >
                  <span className="text-indigo-400">const</span> res = <span className="text-indigo-400">await</span> <span className="text-sky-400">fetch</span>(<span className="text-emerald-300">&quot;https://stenly.org/api/ai/honcho?text=&quot;</span> + <span className="text-sky-400">encodeURIComponent</span>(<span className="text-emerald-300">&quot;halo&quot;</span>));<br/>
                    ...
                    >
                      <div className="space-y-1.5">
                        ...
                        <div className="text-sky-400 text-[10px] font-bold leading-relaxed border-l-2 border-sky-900/50 pl-3 py-0.5 break-all">
                          https://stenly.org/api/ai/honcho?text={textParam}
                        </div>
                      </div>
