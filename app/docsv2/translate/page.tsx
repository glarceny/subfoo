...
const Header = () => (
  <div className="bg-white border-b border-slate-200 py-2 px-4 sticky top-0 z-50 flex items-center justify-between shadow-sm">
    <div className="flex items-center gap-2">
      <div className="w-4 h-4 bg-black rounded flex items-center justify-center text-white font-black text-[8px] shadow-inner">T</div>
      <div className="flex items-center gap-1 font-extrabold text-slate-900 text-[10px] tracking-tight uppercase">
        ...
        <span className="text-slate-300">/</span>
        <span className="text-black">Translate & TTS</span>
      </div>
    </div>
  ...
  <div className="bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm">
    <table className="w-full text-left border-collapse">
      ...
      <tbody className="text-[9px]">
        <ParamRow param="text" type="String" desc="Teks yang ingin diproses." required />
        <ParamRow param="to" type="String" desc="Kode bahasa tujuan terjemahan / TTS (misal: id, en, ja)." required />
        <ParamRow param="mode" type="String" desc="Pilih: 'text-to-text' atau 'text-to-audio'. Default: 'text-to-text'." />
        <ParamRow param="from" type="String" desc="Kode bahasa asal teks. Default: 'auto'." />
        <ParamRow param="autoTranslate" type="Boolean" desc="Mode text-to-audio: Translate otomatis sebelum di TTS? Default: true." />
      </tbody>
    </table>
  </div>
);
...
export default function TranslateDocs() {
  const [textParam, setTextParam] = useState("Halo, namaku stenly. Hari ini aku sedang membuat text to speech bahasa Jepang.");
  const [modeParam, setModeParam] = useState("text-to-audio");
  const [toParam, setToParam] = useState("ja");
  const [fromParam, setFromParam] = useState("id");
  const [autoTranslateParam, setAutoTranslateParam] = useState("true");
  
  const [isLoading, setIsLoading] = useState(false);
  const [response, setResponse] = useState<any>(null);
  const [audioUrl, setAudioUrl] = useState<string | null>(null);
  const [copied, setCopied] = useState<string | null>(null);
  const [activeTab, setActiveTab] = useState<'payload' | 'response' | 'info'>('payload');
  ...
  const handleExecute = async () => {
    if (isLoading || !textParam || !toParam) return;
    ...
    try {
      const url = new URL("/api/tools/translate", window.location.origin);
      ...
      url.searchParams.set("autoTranslate", autoTranslateParam);
      ...
      if (data && data.Mode === "text-to-audio" && data.Result_url) {
        // Create an internal playable URL by replacing stenly.org with the actual origin
        const playableUrl = data.Result_url.replace("https://stenly.org", window.location.origin);
        setAudioUrl(playableUrl); // We'll reuse the state variable for the URL
      }
      ...
      <main className="max-w-[700px] mx-auto px-4 py-8 space-y-10 pb-20">
        ...
        <section className="space-y-4">
          <div className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-gray-100 border border-gray-200/50 rounded-[4px] text-[7px] font-black text-black uppercase tracking-[0.2em] shadow-sm">
            <Languages className="w-2.5 h-2.5" /> TRANSLATOR & TTS NODE
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-[20px] font-black text-slate-900 tracking-tight uppercase italic leading-none flex items-center gap-2">
              Translate & TTS <span className="text-slate-400">Node</span>
            </h1>
          </div>

          <p className="text-[12px] text-slate-600 leading-relaxed max-w-xl pb-2">
            Tool untuk menerjemahkan teks menggunakan Google Translate API X, serta konversi Teks menjadi Suara Audio MP3.
          </p>

          <EndpointBox method="GET" url="https://stenly.org/api/tools/translate" />
        </section>
             ...
             <div className="px-3 py-2.5 bg-[#0a0a0c] border-b border-slate-900 flex items-center justify-between shrink-0 overflow-x-auto scrollbar-hide">
                ...
                <div className="flex bg-[#050505] rounded-[4px] border border-slate-900 p-0.5 ml-4">
                   ...
                   <button 
                     ...
                     className={`px-2.5 py-1 rounded-[3px] text-[7px] font-black uppercase tracking-[0.2em] transition-all whitespace-nowrap ${activeTab === 'info' ? 'bg-[#151515] text-amber-400 border border-slate-800 shadow-sm' : 'text-slate-600 hover:text-slate-400'}`}
                   >LANG.INFO</button>
                </div>
             </div>
                ...
                <AnimatePresence mode="wait">
                  {activeTab === 'info' ? (
                     <motion.div 
                      ...
                      className="space-y-4"
                    >
                        <div className="text-slate-700 text-[7px] uppercase tracking-[0.3em] font-black"># BAHASA UMUM YANG DIDUKUNG TTS & TRANSLATE</div>
                        <div className="text-slate-400 text-[9px] leading-loose whitespace-pre-wrap font-mono pr-4">
                          auto = Auto Detect / Deteksi Otomatis, khusus FROM<br/>
                          id   = Indonesian / Bahasa Indonesia<br/>
                          en   = English / Bahasa Inggris<br/>
                          ja   = Japanese / Bahasa Jepang<br/>
                          ko   = Korean / Bahasa Korea<br/>
                          zh-cn = Chinese Simplified / China Sederhana<br/>
                          zh-tw = Chinese Traditional / China Tradisional<br/>
                          ar   = Arabic / Bahasa Arab<br/>
                          ms   = Malay / Bahasa Melayu<br/>
                          jv   = Javanese / Bahasa Jawa<br/>
                          su   = Sundanese / Bahasa Sunda<br/>
                          es   = Spanish / Bahasa Spanyol<br/>
                          fr   = French / Bahasa Prancis<br/>
                          de   = German / Bahasa Jerman<br/>
                          it   = Italian / Bahasa Italia<br/>
                          ru   = Russian / Bahasa Rusia<br/>
                          th   = Thai / Bahasa Thailand<br/>
                          vi   = Vietnamese / Bahasa Vietnam
                        </div>
                    </motion.div>
                  ) : activeTab === 'payload' ? (
                    <motion.div 
                    ...
                    >
                      <div className="space-y-1.5">
                        ...
                        <div className="text-sky-400 text-[10px] font-bold leading-relaxed border-l-2 border-sky-900/50 pl-3 py-0.5 break-all">
                          https://stenly.org/api/tools/translate?text={encodeURIComponent(textParam)}&to={toParam}&mode={modeParam}&from={fromParam}&autoTranslate={autoTranslateParam}
                        </div>
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <pre className="text-[10px] font-bold leading-loose p-5 bg-[#0a0a0c] rounded-xl border border-slate-900 shadow-inner overflow-x-auto whitespace-pre-wrap">
                          ...
                          &nbsp;&nbsp;<span className="text-indigo-400">&quot;text&quot;</span><span className="text-slate-600">:</span> <span className="text-emerald-500">&quot;{textParam}&quot;</span>,<br />
                          &nbsp;&nbsp;<span className="text-indigo-400">&quot;to&quot;</span><span className="text-slate-600">:</span> <span className="text-emerald-500">&quot;{toParam}&quot;</span>,<br />
                          &nbsp;&nbsp;<span className="text-indigo-400">&quot;mode&quot;</span><span className="text-slate-600">:</span> <span className="text-emerald-500">&quot;{modeParam}&quot;</span>,<br />
                          &nbsp;&nbsp;<span className="text-indigo-400">&quot;from&quot;</span><span className="text-slate-600">:</span> <span className="text-emerald-500">&quot;{fromParam}&quot;</span>,<br />
                          &nbsp;&nbsp;<span className="text-indigo-400">&quot;autoTranslate&quot;</span><span className="text-slate-600">:</span> <span className="text-emerald-500">&quot;{autoTranslateParam}&quot;</span><br />
                    ...
                    >
                      ...
                      {isLoading ? (
                         ...
                         <div className="space-y-3">
                           ...
                           {response.Mode === "text-to-audio" && audioUrl && (
             ...
             <div className="p-4 bg-[#0a0a0c] border-t border-slate-900 shrink-0 space-y-4">
                ...
                <div className="flex items-center justify-center pt-1">
                   <button 
                     ...
                     disabled={isLoading || !textParam || !toParam}
                ...
                <div className="flex flex-col gap-3 px-2">
                   <div className="flex items-center gap-2 flex-1">
                     <span className="text-slate-600 text-[10px] font-bold uppercase tracking-widest w-16">TEXT</span>
                     <input 
                       type="text"
                       value={textParam}
                       ...
                       placeholder="Halo, namaku stenly..."
                     />
                   </div>
                   
                   <div className="flex gap-4">
                     <div className="flex items-center gap-2 flex-1">
                       <span className="text-slate-600 text-[10px] font-bold uppercase tracking-widest w-16">MODE</span>
                       <select 
                         value={modeParam}
                         onChange={(e) => setModeParam(e.target.value)}
                     ...
                     <div className="flex items-center gap-2 flex-1">
                       <span className="text-slate-600 text-[10px] font-bold uppercase tracking-widest w-16">FROM</span>
                       <input 
                         type="text"
                         value={fromParam}
                         onChange={(e) => setFromParam(e.target.value)}
                         className="bg-transparent border-b border-slate-800 text-[10px] font-bold text-slate-300 outline-none w-full transition-all focus:border-sky-600 placeholder:text-slate-700 pb-1"
                         placeholder="auto, id, en"
                       />
                     </div>
                     <div className="flex items-center gap-2 flex-1">
                       <span className="text-slate-600 text-[10px] font-bold uppercase tracking-widest w-16">TO</span>
                       <input 
                         type="text"
                         value={toParam}
                   ...
                   {modeParam === "text-to-audio" && (
                     <div className="flex items-center gap-2 flex-1">
                       <span className="text-slate-600 text-[10px] font-bold uppercase tracking-widest w-24">AUTO TRANSLATE</span>
                       <select 
                         value={autoTranslateParam}
                         onChange={(e) => setAutoTranslateParam(e.target.value)}
                         className="bg-[#111] border border-slate-800 text-[10px] font-bold text-slate-300 outline-none p-1 rounded transition-all focus:border-sky-600"
                       >
