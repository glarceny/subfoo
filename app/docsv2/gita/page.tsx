...
const Header = () => (
  <div className="bg-white border-b border-slate-200 py-2 px-4 sticky top-0 z-50 flex items-center justify-between shadow-sm">
    <div className="flex items-center gap-2">
      <div className="w-4 h-4 bg-black rounded flex items-center justify-center text-white font-black text-[8px] shadow-inner">G</div>
      <div className="flex items-center gap-1 font-extrabold text-slate-900 text-[10px] tracking-tight uppercase">
        ...
        <span className="text-slate-300">/</span>
        <span className="text-black">Gita AI</span>
      </div>
    </div>
  ...
  <div className="bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm">
    <table className="w-full text-left border-collapse">
      ...
      <tbody className="text-[9px]">
        <ParamRow param="text" type="String" desc="Pesan pertanyaan ke Gita AI Assistant." required />
      </tbody>
    </table>
  </div>
);
...
export default function GitaDocs() {
  const [textParam, setTextParam] = useState("halo, ceritakan tentang dharma");
  ...
  const handleExecute = async () => {
    ...
    try {
      const url = new URL("/api/ai/gita", window.location.origin);
        ...
        <section className="space-y-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-[20px] font-black text-slate-900 tracking-tight uppercase italic leading-none flex items-center gap-2">
              Gita AI <span className="text-slate-400">Node</span>
            </h1>
          </div>

          <p className="text-[12px] text-slate-600 leading-relaxed max-w-xl pb-2">
            Endpoint GITA AI - Chat with Bhagavad Gita AI Assistant.
          </p>

          <EndpointBox method="GET" url="https://stenly.org/api/ai/gita" />
        </section>
            ...
            <div className="space-y-4 flex-1 min-w-0">
              ...
              <div className="space-y-4 w-full pt-1">
                <MicroCodeBlock 
                  ...
                  rawCode={`curl "https://stenly.org/api/ai/gita?text=halo"`}
                  copyId="curl"
                  copied={copied}
                  onCopy={copyCode}
                >
                  <span className="text-emerald-400">curl</span> <span className="text-emerald-300">&quot;https://stenly.org/api/ai/gita?text=halo&quot;</span>
                </MicroCodeBlock>

                <MicroCodeBlock 
                  label="Node.js (Fetch)"
                  rawCode={`const res = await fetch("https://stenly.org/api/ai/gita?text=" + encodeURIComponent("halo"));\nconst data = await res.json();\nconsole.log(data);`}
                  copyId="js"
                  copied={copied}
                  onCopy={copyCode}
                >
                  <span className="text-indigo-400">const</span> res = <span className="text-indigo-400">await</span> <span className="text-sky-400">fetch</span>(<span className="text-emerald-300">&quot;https://stenly.org/api/ai/gita?text=&quot;</span> + <span className="text-sky-400">encodeURIComponent</span>(<span className="text-emerald-300">&quot;halo&quot;</span>));<br/>
                    ...
                    >
                      <div className="space-y-1.5">
                        ...
                        <div className="text-sky-400 text-[10px] font-bold leading-relaxed border-l-2 border-sky-900/50 pl-3 py-0.5 break-all">
                          https://stenly.org/api/ai/gita?text={textParam}
                        </div>
                      </div>
