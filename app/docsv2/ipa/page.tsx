...
const Header = () => (
  <div className="bg-white border-b border-slate-200 py-2 px-4 sticky top-0 z-50 flex items-center justify-between shadow-sm">
    <div className="flex items-center gap-2">
      <div className="w-4 h-4 bg-black rounded flex items-center justify-center text-white font-black text-[8px] shadow-inner">I</div>
      <div className="flex items-center gap-1 font-extrabold text-slate-900 text-[10px] tracking-tight uppercase">
        ...
        <span className="text-slate-300">/</span>
        <span className="text-black">IPA</span>
      </div>
    </div>
...
const EndpointBox = ({ method, url }: { method: string, url: string }) => (
  <div className="bg-[#050505] rounded-lg p-2.5 flex items-center justify-between shadow-lg border border-slate-800 mb-2">
    <div className="flex items-center gap-2 overflow-hidden">
      <div className="bg-sky-500/10 text-sky-400 px-2 py-0.5 rounded-[3px] text-[8px] font-black tracking-[0.2em] uppercase border border-sky-500/20 shrink-0">
        {method}
      </div>
      <code className="text-[10px] font-bold text-slate-200 tracking-wide font-mono truncate">
        {url}
      </code>
    </div>
    <div className="flex items-center gap-1.5 text-[7px] font-extrabold text-slate-500 uppercase tracking-widest px-2 py-0.5 border border-slate-800 rounded bg-slate-900/50 shrink-0 ml-2">
      <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></div> Live
    </div>
  </div>
);

const DenseTable = ({ isDetail = false }: { isDetail?: boolean }) => (
  <div className="bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm mb-4">
    <table className="w-full text-left border-collapse">
      ...
      <tbody className="text-[9px]">
        {isDetail ? (
          <ParamRow param="slug" type="String" desc="Slug artikel IPA (contoh: dampak-pemanasan-global)." required />
        ) : (
          <>
            <ParamRow param="q" type="String" desc="Kata kunci pencarian pelajaran IPA." required />
            <ParamRow param="page" type="Number" desc="Halaman pencarian (default: 1)." />
          </>
        )}
      </tbody>
    </table>
...
const MicroCodeBlock = ({ rawCode, children, label, copyId, copied, onCopy }: { rawCode: string, children: React.ReactNode, label: string, copyId: string, copied: string | null, onCopy: (text: string, id: string) => void }) => (
  <div className="space-y-1 w-full flex flex-col justify-start mb-3">
...
export default function IPADocs() {
  const [qParam, setQParam] = useState("CO2");
  const [pageParam, setPageParam] = useState("1");
  const [endpointType, setEndpointType] = useState<'search' | 'detail'>('search');
  ...
  const handleExecute = async () => {
    ...
    if (endpointType === 'search' && !qParam) return;
    if (endpointType === 'detail' && !qParam) return;
    ...
    try {
      ...
      if (endpointType === 'search') {
        const url = new URL("/api/search/ipa", window.location.origin);
        ...
        const url = new URL("/api/search/ipa/detail", window.location.origin);
        url.searchParams.set("slug", qParam);
        urlStr = url.toString();
      }
      ...
      <main className="max-w-[700px] mx-auto px-4 py-8 space-y-10 pb-20">
        ...
        <section className="space-y-4">
          <div className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-gray-100 border border-gray-200/50 rounded-[4px] text-[7px] font-black text-black uppercase tracking-[0.2em] shadow-sm">
            <BookOpen className="w-2.5 h-2.5" /> EDU NODE
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-[20px] font-black text-slate-900 tracking-tight uppercase italic leading-none flex items-center gap-2">
              IPA <span className="text-slate-400">Node</span>
            </h1>
          </div>

          <p className="text-[12px] text-slate-600 leading-relaxed max-w-xl pb-2">
            API Scraping pelajaran IPA dari ipa.pelajaran.co.id. Dapatkan artikel, materi pelajaran, dan hasil pencarian.
          </p>
        </section>

        <section className="space-y-5">
            <h3 className="text-[11px] font-black text-slate-900 uppercase tracking-widest border-b border-slate-200/60 pb-1.5 mb-2">Endpoint: Search</h3>
            <EndpointBox method="GET" url="https://stenly.org/api/search/ipa" />
            <DenseTable isDetail={false} />

            <h3 className="text-[11px] font-black text-slate-900 uppercase tracking-widest border-b border-slate-200/60 pb-1.5 mb-2 mt-6">Endpoint: Detail</h3>
            <EndpointBox method="GET" url="https://stenly.org/api/search/ipa/detail" />
            <DenseTable isDetail={true} />
        </section>
        ...
        <section className="space-y-5">
          ...
          <div className="flex gap-3 items-start w-full">
            ...
            <div className="space-y-4 flex-1 min-w-0">
              ...
              <p className="text-[10px] text-slate-600 leading-relaxed -mt-2">Lakukan HTTP GET request ke endpoint pencarian atau detail.</p>
              
              <div className="space-y-4 w-full pt-1">
                <MicroCodeBlock 
                  label="Terminal / Bash (cURL - Search)"
                  rawCode={`curl "https://stenly.org/api/search/ipa?q=CO2&page=1"`}
                  copyId="curl_search"
                  copied={copied}
                  onCopy={copyCode}
                >
                  <span className="text-emerald-400">curl</span> <span className="text-emerald-300">&quot;https://stenly.org/api/search/ipa?q=CO2&amp;page=1&quot;</span>
                </MicroCodeBlock>
                
                <MicroCodeBlock 
                  label="Terminal / Bash (cURL - Detail)"
                  rawCode={`curl "https://stenly.org/api/search/ipa/detail?slug=pengertian-unsur-senyawa-dan-campuran"`}
                  copyId="curl_detail"
                  copied={copied}
                  onCopy={copyCode}
                >
                  <span className="text-emerald-400">curl</span> <span className="text-emerald-300">&quot;https://stenly.org/api/search/ipa/detail?slug=pengertian-unsur-senyawa-dan-campuran&quot;</span>
                </MicroCodeBlock>
              </div>
            </div>
          </div>
                    ...
                    >
                      <div className="space-y-1.5">
                        ...
                        <div className="text-sky-400 text-[10px] font-bold leading-relaxed border-l-2 border-sky-900/50 pl-3 py-0.5 break-all">
                          {endpointType === 'search' 
                            ? `https://stenly.org/api/search/ipa?q=${qParam}&page=${pageParam}`
                            : `https://stenly.org/api/search/ipa/detail?slug=${qParam}`
                          }
                        </div>
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <pre className="text-[10px] font-bold leading-loose p-5 bg-[#0a0a0c] rounded-xl border border-slate-900 shadow-inner overflow-x-auto whitespace-pre-wrap">
                          ...
                          {endpointType === 'search' ? (
                            <>
                              &nbsp;&nbsp;<span className="text-indigo-400">&quot;q&quot;</span><span className="text-slate-600">:</span> <span className="text-emerald-500">&quot;{qParam}&quot;</span><br />
                              &nbsp;&nbsp;<span className="text-indigo-400">&quot;page&quot;</span><span className="text-slate-600">:</span> <span className="text-sky-500">{pageParam}</span><br />
                            </>
                          ) : (
                            <>
                              &nbsp;&nbsp;<span className="text-indigo-400">&quot;slug&quot;</span><span className="text-slate-600">:</span> <span className="text-emerald-500">&quot;{qParam}&quot;</span><br />
                            </>
                          )}
             ...
             <div className="p-4 bg-[#0a0a0c] border-t border-slate-900 shrink-0 space-y-4">
                ...
                <div className="flex items-center justify-center pt-1 gap-2">
                   <select 
                     ...
                     className="bg-[#050505] border border-slate-800 text-slate-300 text-[10px] font-bold outline-none p-2 rounded hover:border-slate-700 transition-colors"
                   >
                   ...
                   <button 
                     ...
                     disabled={isLoading || !qParam}
                     className="flex-1 bg-sky-500/10 border border-sky-500/30 rounded py-2 flex items-center justify-center gap-2.5 group hover:bg-sky-500/20 active:scale-[0.98] transition-all disabled:opacity-50"
                   >
                ...
                <div className="flex items-center justify-between gap-2 px-2">
                   <div className="flex items-center gap-2 flex-1">
                     ...
                     <input 
                       ...
                       placeholder={endpointType === 'search' ? "CO2" : "pengertian-unsur-senyawa-dan-campuran"}
                     />
                   </div>
                   {endpointType === 'search' && (
                     <div className="flex items-center gap-2 w-20">
                       ...
                       <input 
                         ...
                         className="bg-transparent border-b border-slate-800 text-[10px] font-bold text-slate-300 outline-none w-full transition-all focus:border-sky-600 placeholder:text-slate-700 pb-1 text-center"
                         placeholder="1"
                         min="1"
                       />
                     </div>
